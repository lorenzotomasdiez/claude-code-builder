export const meta = {
  name: 'qa-suite-pro-computer-use',
  description: 'Full QA of a service/area: architect a layered code-test strategy AND derive UI user stories, engineer the missing code tests and run them, verify coverage, then drive a real headed Chrome (Claude computer-use / in-Chrome) through each UI story one at a time with screenshots, and report code + UI results',
  phases: [
    ...
    { title: 'Browse', detail: 'drive a real headed Chrome through each UI story sequentially (single shared instance), screenshot every step, capture pass/fail and console errors' },
...
const runDir = `qa-runs/qa-suite-pro-computer-use-${runId}`
...
const scope = await agent(
  ...
  { agentType: 'qa-suite-pro-computer-use-scoper', schema: SCOPE_SCHEMA }
)
...
const strategy = await agent(
  ...
  { agentType: 'qa-suite-pro-computer-use-architect', schema: STRATEGY_SCHEMA }
)
...
while (round < MAX_ROUNDS) {
  ...
  engineering = await agent(
    ...
    { agentType: 'qa-suite-pro-computer-use-engineer', phase: 'Implement', schema: ENGINEER_SCHEMA }
  )
  ...
  coverage = await agent(
    ...
    { agentType: 'qa-suite-pro-computer-use-coverage-critic', phase: 'Verify', schema: COVERAGE_SCHEMA }
  )
...
// Single shared Chrome instance - stories run SEQUENTIALLY, never in parallel.
let browserResults = []
if (strategy.uiStories.length > 0) {
  ...
  const authored = await agent(
    ...
    { agentType: 'qa-suite-pro-computer-use-story-author', phase: 'Author stories', schema: AUTHOR_SCHEMA }
  )
  log(`Authored ${authored.stories.length} story file entries at ${authored.storiesFile}`)

  phase('Browse')
  for (const story of strategy.uiStories) {
    const result = await agent(
      `Execute this UI user story in a real headed Chrome (Claude computer-use / in-Chrome), step by step, screenshotting after every step into ${runDir}/screenshots/${story.slug}/. This is a single shared browser instance - you are the only story running right now. Report pass/fail per the browser-runner contract, capturing JS console errors on failure, and leave the browser in a clean state for the next story.\n\nStory name: ${story.name}\nStart URL: ${story.url}\nSteps:\n${story.workflow}`,
      { agentType: 'qa-suite-pro-computer-use-browser-runner', label: `browse:${story.slug}`, phase: 'Browse', schema: BROWSER_SCHEMA }
    )
    if (result) browserResults.push(result)
    log(`Story ${story.slug}: ${result ? result.status : 'no result'}`)
  }
...
const report = await agent(
  ...
  { agentType: 'qa-suite-pro-computer-use-reporter' }
)
