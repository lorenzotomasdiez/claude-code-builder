...
`workflow-dry-run` (repo-internal, not part of the installable catalog) is a deterministic, no-LLM dry run of a workflow script's own orchestration control flow - it mocks `agent()`/`parallel()`/`pipeline()` with data shaped from the real schema each call already declares, makes every 3rd `agent()` call fail (simulating a transient subagent failure), and runs the real script once to see whether it degrades gracefully or crashes with an unguarded null read. `node workflow-dry-run/workflow-dry-run.mjs --all` found 3 real bugs of exactly that kind on its first run against the whole library (`bug-hunter`, `qa-suite-pro`, `tdd-blueprint`, none caught by the earlier `NULL_SAFETY_AFTER_FANOUT.md` audit because none involve a `parallel()`/`pipeline()` call site) - all three fixed, see `workflow-dry-run/README.md` and `NULL_SAFETY_AFTER_FANOUT.md`'s "Extension" section.
