...
**Schema-description retrofit (2026-07-28):** `FINDINGS_SCHEMA`'s `severity` field and `VERDICT_SCHEMA`'s `verdict` field now carry a `description` stating the calibration between their enum values (see `SCHEMA_DESIGN.md`). Not re-run as a full fan-out for this change - a `description` string adds no new required field, enum value, or control-flow branch, so it cannot change what the SDK accepts; `context-bloat-forensics` (see its README) was run live end-to-end with the same kind of change to prove the SDK accepts `description` on an `enum` field without incident. `node scripts/validate-workflow.mjs code-review` still passes.

**Prompt-cache-ordering retrofit (2026-07-28):** `reviewPrompt(lens)` previously interpolated the lens name before the shared scope brief and diff, breaking cache-prefix stability across the 5 parallel lens calls (see `PROMPT_CACHE_ORDERING.md`). Reordered so the shared, tagged `<scope>`/`<diff>` payload comes first and the lens instruction last - matching this file's own `verifyPrompt`, which already used this ordering. Verified by invoking the real `code-review-correctness-lens` agent definition directly via the `Agent` tool with the new prompt shape, against a trivial two-line diff with one planted bug (a `.find(...).name` call with no null guard): the agent returned a schema-valid `FINDINGS_SCHEMA` object correctly flagging the crash as `critical`. A full 5-lens fan-out was not re-run - the change is prompt-string ordering only, no schema/control-flow change, and `node scripts/validate-workflow.mjs code-review` still passes.
