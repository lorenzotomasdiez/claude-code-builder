...
Return your lens name, a list of findings (each sourced or labeled), a list of risks, and - for the technical lens - a list of dependencies (can be empty for other lenses). Keep each finding/risk/dependency to one concrete point rather than a bundled paragraph - if it needs more than a couple sentences, it is probably more than one item.
