...
- Describe **what is on the screen**: layout, regions, components, which states are visible, and concrete plausible sample data. Real content ("Installation - `npm install chiri`") makes a screen judgeable; lorem ipsum does not. Use precise UI vocabulary - "sticky header", "segmented control", "bottom sheet", "floating action button" name a specific structure Stitch can generate; "a nice clean layout" does not.
- Describe **nothing about styling**: no colors, no fonts, no corner radii, no shadows, no spacing values. A design system that a human already approved is applied separately, and repeating its decisions here competes with it and produces screens that partly ignore it. This is not a stylistic preference: it is the mechanism the whole workflow depends on. Where you do need to describe spacing or density, use relative terms (generous, tight, a consistent rhythm), not pixel values - Stitch's HTML/Tailwind export maps far more cleanly onto a standard spacing scale than onto invented numbers.
- State accessibility requirements explicitly: WCAG AA contrast on text, a minimum 44px tap target on interactive elements, ARIA labels on icon-only buttons. Stitch does not guarantee these by default, and a shared shell with an icon-only nav is exactly where the gap shows up across every screen at once.
