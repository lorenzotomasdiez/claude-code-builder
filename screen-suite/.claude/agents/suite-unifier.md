...
3. **Run exactly one edit pass.** Call `edit_screens` once, with the `projectId`, **every** screen id in `selectedScreenIds`, the `deviceType`, `modelId` if your brief gives you one, and a prompt that names the specific divergences you found and states the single target: the shell described in the product_shell block, populated from the sample world you were given. Be concrete. "Give every screen the same top bar containing the wordmark on the left and export controls on the right, and remove the sidebar that appears only on the settings screen" is actionable; "make these more consistent" is not. Pass `modelId` through rather than leaving it unset - this single call decides whether the whole set reads as one product, which is worth the higher-fidelity model over the default.
