...
const initialResults = await parallel(PANEL.map(p => () =>
  agent(
    ...
    `You are proposing, not debating yet. Propose your approach to the brief above from your lens only, at the length the proposal actually needs - other seats cover the ground outside your lens, so flag it rather than designing it.`,
...
const proposal = await agent(
  ...
  `Synthesize the panel debate above into one coherent technical solution proposal, following the house section structure. Write only what the panel and the brief support: no new technology choices, no sections beyond the structure, and no section padded past the substance the debate actually produced.`,
