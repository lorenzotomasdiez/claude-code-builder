...
**Addendum**: `workflow-dry-run/workflow-dry-run.mjs` (a later, repo-wide no-LLM control-flow check) found that the Implement phase read `engineering.testsWritten` with no guard against `agent()` resolving `null` on a transient subagent failure - a real crash-on-null-result bug. Fixed with a guard-and-throw clause; see `NULL_SAFETY_AFTER_FANOUT.md`'s "Extension" section for detail. `node workflow-dry-run/workflow-dry-run.mjs qa-suite-pro` now passes clean.
