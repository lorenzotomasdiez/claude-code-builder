...
**Schema-description retrofit (2026-07-28):** `FINDING.severity` and `VERDICT_SCHEMA.verdict` now carry a `description` stating the calibration between their enum values (see `SCHEMA_DESIGN.md`). Not re-run as a full fan-out for this change - a `description` string adds no new required field, enum value, or control-flow branch, so it cannot change what the SDK accepts; `context-bloat-forensics` (see its README) was run live end-to-end with the same kind of change to prove the SDK accepts `description` on an `enum` field without incident. `node scripts/validate-workflow.mjs security-audit` still passes.

**Prompt-cache-ordering retrofit (2026-07-28):** `auditPrompt(lens)` previously interpolated the lens name before the shared scope brief and target, breaking cache-prefix stability across the 5 parallel lens calls (see `PROMPT_CACHE_ORDERING.md`). Reordered so the shared, tagged `<scope>`/`<target>` payload comes first and the lens instruction last - matching this file's own `verifyPrompt`, which already used this ordering. Verified via the same mechanism proven live for `code-review` (see its README): a single targeted `Agent` tool call against the reordered prompt shape returned schema-valid, correctly-flagged output. A full 5-lens fan-out was not re-run for this package - the change is prompt-string ordering only, no schema/control-flow change, and `node scripts/validate-workflow.mjs security-audit` still passes.
